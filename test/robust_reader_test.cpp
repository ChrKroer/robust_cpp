...
#include "./../src/online_convex_optimization/follow_the_perturbed_leader.h"
...
void solve_with_ftpl(vector_d x) {
  ...
  follow_the_perturbed_leader rms(unc->get_domain(), *unc);
  rms.function_max(maximizer_violation_amount);
  ...
  EXPECT_NEAR(maximizer_violation_amount, oco_violation_amount, 1e-2);
}
